...
namespace EventStore.Common.Options
{
    public class EventStoreOptions
    {
        ...
        public static IEnumerable<OptionSource> Update(IEnumerable<OptionSource> optionSources)
        {
            ...
            var conflictedOptions = optionSources.DeterminePotentialPrecedenceIssues(effectiveOptions);
            ...
            if (conflictedOptions.Count() > 0)
            {
                ...
                var affectedOptions = effectiveOptions.Where(x => conflictedOptions.Contains(y => y.Name.Equals(x.Name)));
                string explanationOfFailure = String.Format("The following updates ({0}{1}) ", String.Join(",", conflictedOptions.Select(x => nameSourceAndValue(x))), Environment.NewLine);
                explanationOfFailure += String.Format("will be ignored due to the current configuration taking precedence over the updates being stored in the Config File. ({0})", String.Join(",", affectedOptions.Select(x => nameSourceAndValue(x))));
